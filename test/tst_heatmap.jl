...
@testset "axis scaling" begin
    ...
    test_ref("references/heatmap/scaling_$(size(x, 1))x$(size(x, 2))_xscale_0.1.txt", @show_col(p, :displaysize=>T_SZ))
    p = @inferred heatmap(x, xscale=0.1, xoffset=-0.5)
    test_ref("references/heatmap/scaling_$(size(x, 1))x$(size(x, 2))_xscale_0.1_xoffset-0.5.txt", @show_col(p, :displaysize=>T_SZ))
    p = @inferred heatmap(x, yscale=0.1)
    test_ref("references/heatmap/scaling_$(size(x, 1))x$(size(x, 2))_yscale_0.1.txt", @show_col(p, :displaysize=>T_SZ))
    p = @inferred heatmap(x, yscale=0.1, yoffset=-0.5)
    test_ref("references/heatmap/scaling_$(size(x, 1))x$(size(x, 2))_yscale_0.1_yoffset-0.5.txt", @show_col(p, :displaysize=>T_SZ))
...
@testset "axis limits" begin
    ...
    test_ref("references/heatmap/limits_$(size(x, 1))x$(size(x, 2))_xscale_0.01_xlim_0_0.1.txt", @show_col(p, :displaysize=>T_SZ))
    p = @inferred heatmap(x, ylim=[0, 0.1], yscale=0.01)
    test_ref("references/heatmap/limits_$(size(x, 1))x$(size(x, 2))_yscale_0.01_ylim_0_0.1.txt", @show_col(p, :displaysize=>T_SZ))
...
@testset "parameters" begin
    ...
    test_ref("references/heatmap/parameters_10x10_xscale_inferno.txt", @show_col(p, :displaysize=>T_SZ))
    seed!(RNG, 1337)
    p = @inferred heatmap(randn(RNG, 10, 11), xscale=0.1, colormap=:inferno)
    test_ref("references/heatmap/parameters_10x11_xscale_inferno.txt", @show_col(p, :displaysize=>T_SZ))
    seed!(RNG, 1337)
    p = @inferred heatmap(randn(RNG, 10, 10), yscale=1, colormap=:inferno)
    test_ref("references/heatmap/parameters_10x10_yscale_inferno.txt", @show_col(p, :displaysize=>T_SZ))
end
