@testset "aspect ratio" begin
    ...
    test_ref(
        "references/heatmap/fix_aspect_ratio_30x30.txt",
...
@testset "axis scaling" begin
    ...
    test_ref(
        "references/heatmap/scaling_$(size(x, 1))x$(size(x, 2))_xfact_0.1.txt",
        @show_col(p, :displaysize => T_SZ)
    )
    p = @inferred heatmap(x, xfact = 0.1, xoffset = -0.5)
    test_ref(
        "references/heatmap/scaling_$(size(x, 1))x$(size(x, 2))_xfact_0.1_xoffset-0.5.txt",
        @show_col(p, :displaysize => T_SZ)
    )
    p = @inferred heatmap(x, yfact = 0.1)
    test_ref(
        "references/heatmap/scaling_$(size(x, 1))x$(size(x, 2))_yfact_0.1.txt",
        @show_col(p, :displaysize => T_SZ)
    )
    p = @inferred heatmap(x, yfact = 0.1, yoffset = -0.5)
    test_ref(
        "references/heatmap/scaling_$(size(x, 1))x$(size(x, 2))_yfact_0.1_yoffset-0.5.txt",
        @show_col(p, :displaysize => T_SZ)
    )
...
@testset "axis limits" begin
    ...
    test_ref(
        "references/heatmap/limits_$(size(x, 1))x$(size(x, 2))_xfact_0.01_xlim_0_0.1.txt",
        @show_col(p, :displaysize => T_SZ)
    )
    p = @inferred heatmap(x, ylim = [0, 0.1], yfact = 0.01)
    test_ref(
        "references/heatmap/limits_$(size(x, 1))x$(size(x, 2))_yfact_0.01_ylim_0_0.1.txt",
        @show_col(p, :displaysize => T_SZ)
    )
    ...
    test_ref(
        "references/heatmap/limits_$(size(x, 1))x$(size(x, 2))_zlim_0_$zmax.txt",
        @show_col(p, :displaysize => T_SZ)
    )
end
...
@testset "parameters" begin
    ...
    test_ref(
        "references/heatmap/parameters_10x10_xfact_inferno.txt",
        @show_col(p, :displaysize => T_SZ)
    )
    seed!(RNG, 1337)
    p = @inferred heatmap(randn(RNG, 10, 11), xfact = 0.1, colormap = :inferno)
    test_ref(
        "references/heatmap/parameters_10x11_xfact_inferno.txt",
        @show_col(p, :displaysize => T_SZ)
    )
    seed!(RNG, 1337)
    p = @inferred heatmap(randn(RNG, 10, 10), yfact = 1, colormap = :inferno)
    test_ref(
        "references/heatmap/parameters_10x10_yfact_inferno.txt",
        @show_col(p, :displaysize => T_SZ)
    )
end
