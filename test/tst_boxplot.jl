@testset "default" begin
    ...
    p = boxplot("series1", [1,2,3,4,5], title = "Test", xlim = (-1,8), color = :blue, width = 50, border = :solid, xlabel="foo")
    @test_reference(
...
@testset "scaling" begin
    for (i, max_x) in enumerate([5,6,10,20,40])
        p = boxplot([1,2,3,4,5], xlim=(0,max_x))
        @test_reference(
...
@testset "multi-series" begin
    p = boxplot(["one", "two"], [[1,2,3,4,5], [2,3,4,5,6,7,8,9]], title="Multi-series", xlabel="foo", color=:yellow)
    @test_reference(
        "references/boxplot/multi1.txt",
        @io2str(show(IOContext(::IO, :color=>true), p)),
        render = BeforeAfterFull()
    )
    @test boxplot!(p, "one more", [-1,2,3,4,11]) === p
    @test_reference(
        "references/boxplot/multi2.txt",
        @io2str(show(IOContext(::IO, :color=>true), p)),
        render = BeforeAfterFull()
    )
    @test boxplot!(p, [4,2,2.5,4,14], name = "last one") === p
    @test_reference(
