...
@testset "hist params" begin
    ...
    @test_logs (:warn, r"`symb`.+deprecated") @inferred histogram(
        x,
        symb="#",
        closed=:right,
    )
    p = @test_logs (:warn, r"`bins`.+deprecated") @inferred histogram(
        x,
        bins=5,
        closed=:right,
    )
...
@testset "keyword arguments" begin
    ...
    p = @inferred histogram(x, title="Green color", color=(50, 100, 50))
