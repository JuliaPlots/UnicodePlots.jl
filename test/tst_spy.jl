@testset "sizing" begin
    ...
    test_ref("references/spy/default_overdrawn.txt", @show_col(p, :displaysize=>TERM_SIZE))
    seed!(RNG, 1337)
    p = @inferred spy(stable_sprand(RNG, 200, 200, .001))
    test_ref("references/spy/default_200x200_normal.txt", @show_col(p, :displaysize=>TERM_SIZE))
    test_ref("references/spy/default_200x200_normal_nocolor.txt", @show_nocol(p, :displaysize=>TERM_SIZE))
    seed!(RNG, 1337)
    p = @inferred spy(Matrix(stable_sprand(RNG, 200, 200, .001)))
    test_ref("references/spy/default_200x200_normal.txt", @show_col(p, :displaysize=>TERM_SIZE))
    seed!(RNG, 1337)
    p = @inferred spy(stable_sprand(RNG, 200, 200, .001), width=10)
    test_ref("references/spy/default_200x200_normal_small.txt", @show_col(p, :displaysize=>TERM_SIZE))
    seed!(RNG, 1337)
    p = @inferred spy(stable_sprand(RNG, 200, 200, .001), height=5)
    test_ref("references/spy/default_200x200_normal_small.txt", @show_col(p, :displaysize=>TERM_SIZE))
    seed!(RNG, 1337)
    p = @inferred spy(stable_sprand(RNG, 200, 200, .001), height=5, width=20)
    test_ref("references/spy/default_200x200_normal_misshaped.txt", @show_col(p, :displaysize=>TERM_SIZE))
end

@testset "parameters" begin
    seed!(RNG, 1337)
    p = @inferred spy(stable_sprand(RNG, 200, 200, .001), color=:green)
    test_ref("references/spy/parameters_200x200_green.txt", @show_col(p, :displaysize=>TERM_SIZE))
    test_ref("references/spy/parameters_200x200_green_nocolor.txt", @show_nocol(p, :displaysize=>TERM_SIZE))
    seed!(RNG, 1337)
    p = spy(stable_sprand(RNG, 200, 200, .001), title="Custom Title", canvas=DotCanvas, border=:ascii)
    test_ref("references/spy/parameters_200x200_dotcanvas.txt", @show_col(p, :displaysize=>TERM_SIZE))
end
