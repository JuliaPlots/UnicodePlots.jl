...
main() = begin
  ...
  exs = (
    ...
    barplot1 = (alt="Barplot", code="""
      barplot(["Paris", "New York", "Moskau", "Madrid"],
              ...
              title="Population")
      """),
    histogram1 = (alt="Barplot", code="histogram(randn(1000) .* .1, nbins=15, closed=:left)"),
    ...
    densityplot1 = (alt="Densityplot", code="""
      ...
      """),
    contourplot1 = (alt="Contourplot", code="contourplot(-3:.01:3, -7:.01:3, (x, y) -> exp(-(x / 2)^2 - ((y + 2) / 4)^2))"),
    ...
    width = (alt="Width", code="lineplot(sin, 1:.5:20, width=60)"),
    height = (alt="Height", code="lineplot(sin, 1:.5:20, height=18)"),
    labels = (alt="Labels", code="lineplot(sin, 1:.5:20, labels=false)"),
  ...
  _Note_: You can use the keyword argument `symbols` to specify the characters that should be used to plot the bars. For example `symbols=['#']`
...
$(examples.decorate)
  ...
  bash gen_imgs.sh
  ```
  will regenerate the images with root (prefix) url $(docs_url).
...
#=
Header{1}("UnicodePlots"),
Paragraph([
  Link(Image("http://img.shields.io/badge/license-MIT-brightgreen.svg?style=flat", "License"), "LICENSE.md"), '\n',
  Link(Image("https://juliaci.github.io/NanosoldierReports/pkgeval_badges/U/UnicodePlots.named.svg", "PkgEval"), "https://juliaci.github.io/NanosoldierReports/pkgeval_badges/U/UnicodePlots.html"), '\n',
  Link(Image("https://github.com/JuliaPlots/UnicodePlots.jl/actions/workflows/ci.yml/badge.svg", "CI"), "https://github.com/JuliaPlots/UnicodePlots.jl/actions/workflows/ci.yml"), '\n',
  Link(Image("https://codecov.io/gh/JuliaPlots/UnicodePlots.jl/branch/master/graphs/badge.svg?branch=master", "Coverage Status"), "https://app.codecov.io/gh/JuliaPlots/UnicodePlots.jl"), '\n',
  Link(Image("https://shields.io/endpoint?url=https://pkgs.genieframework.com/api/v1/badge/UnicodePlots", "UnicodePlots Downloads"), "https://pkgs.genieframework.com?packages=UnicodePlots"), '\n',
  "Advanced Unicode plotting library designed for use in Julia's `REPL`."
]),
...
```
"""
show(io, "text/plain", readme)
tohtml(io, readme)
examples = NamedTuple{keys(exs)}((MD(github, Paragraph(["```julia\n$(e.code)```", "\n", Image("$docs_url/$k.png", e.alt)])) for (k, e) in zip(keys(exs), exs)))
