...
function Plot{C<:Canvas, F<:AbstractFloat}(
    ...
    isInt{T<:Number}(num::T) = isinteger(num) & (typemin(Int) < num < typemax(Int))
    min_x_str = string(isInt(min_x) ? round(Int, min_x, RoundNearestTiesUp) : min_x)
    max_x_str = string(isInt(max_x) ? round(Int, max_x, RoundNearestTiesUp) : max_x)
    min_y_str = string(isInt(min_y) ? round(Int, min_y, RoundNearestTiesUp) : min_y)
    max_y_str = string(isInt(max_y) ? round(Int, max_y, RoundNearestTiesUp) : max_y)
